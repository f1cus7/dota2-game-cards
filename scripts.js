...
const bigLastStory = () => showStory(
      'да, нам наквакали что у него украли аегис. теперь он гоняется за каждым, кого только встретит..',
      'а какая помощь нужна от меня?',
      () => showStory(
        'знаешь, моя дудка не просто бамбук, дикокрылы использовали её для отпугивания темных рас. Представителем таковой и есть он. Я хочу что бы ты меня подкинул на плечи, я сыграю мелодию и надеюсь, его она спугнет. С земли эта грамила боюсь не услышит.',
        'хорошо, садись',
        () => showStory(
          '*громкие шаги*',
          'далее',
          () => showStory(
            ...
            () => showStory(
              ...
              () => showStory(
                ...
                'далее',
                () => showStory(
                  'он тут, у меня, забирай',
                  'далее',
                  () => showStory(
                    '*наклоняется*',
                    'далее',
                    () => showStory(
                      '*залазит вам на плечи, вцепившись пальцами\nиграет на трубке*',
                      'далее',
                      () => showStory(
                        'АААААААААГРХ!',
                        'далее',
                        () => showStory(
                          '*убегает, прижав руки к тому, что наверное является ушами*',
                          'далее',
                          () => showStory(
                            'ПОЛУЧИЛОСЬ!\nПолучилось!',
                            'а#%$&#ть, сработало!',
                            () => showStory(
                              'ну, надеюсь он убежал в своё логово на долго.',
                              'далее',
                              () => {
                                if (currEnd === 2) {
                                  showStory(
                                    'слушай, спасибо тебе за помощь, заглядывай к нам в лягушатник, если станет скучно!',
                                    'далее',
                                    () => showStory(
                                  ...
                                  showStory(
                                    'спасибо тебе за помощь, знаешь, ты меня выручил, и я хочу отплатить тебе тем же, забирай мою дудку!',
                                    'но.. я не умею на ней играть..',
                                    () => showStory(
                                      'правильно, поэтому просто носи её с собой, и она будет тебе пассивно давать 100 золота в секунду',
                                      'воу, спасибо!',
                                      () => showStory(
                                        'слушай, спасибо тебе за помощь, заглядывай к нам в лягушатник, если станет скучно!',
                                    'далее',
        ...
        'только слизью не запачкай',
        () => showStory(
          'я тебя потом в нашем болоте искупаю, в миг чистым станешь!',
          'ну да..',
          () => showStory(
          '*громкие шаги*',
          'далее',
  ...
  switch (currEnd) {
    ...
    case 2:
      showStory(
        ...
        'ты живой!',
        () => showStory(
          'мой сын меня нашёл, благодаря твоей помощи!',
          'а где он?',
          () => showStory(
            'я тут',
            'я рад, что вы друг друга нашли! Теперь возвращаетесь домой?',
            () => showStory(
              'большой, он рядом, он идет в нашу сторону, помоги нам!',
              'это тот, которого ты встретил раньше?',
              () => bigLastStory(),
              null,
              null,
              'big_golov'
      ...
      () => showStory(
        ...
        () => showStory(
          ...
          () => showStory(
            ...
            () => showStory(
              'он тут, идёт на нас!',
              'это тот большой, которого ты встретил раньше?',
              () => bigLastStory(),
              null,
              null,
              'big_golov'
